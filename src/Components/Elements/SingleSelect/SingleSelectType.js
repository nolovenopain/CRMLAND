...
import getSearchTypeOfCustomer from '../../../Api/getSearchTypeOfCustomer';

var timeout = 0;

export default class SingleSelectType extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async search(searchText) {
        if(searchText == '') {
            ...
            const res = await getSearchTypeOfCustomer(
                this.state.token, 
                searchText
            );
            if(res.status == 200) {
    ...
    refreshType(listType) {
        this.setState({ items: listType })
    }
